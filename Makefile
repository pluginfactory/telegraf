...
bucket := dl.influxdata.com/telegraf/nightly
...
docdir ?= $(datarootdir)/doc/telegraf
pkgdir ?= build/dist
...
upload:
	@echo cd build/dist && echo aws s3 sync ./ "s3://$(BUCKET)/" \
		--exclude "*" \
		--include "*.tar.gz" \
		--include "*.deb" \
		--include "*.rpm" \
...
%windows_i386.zip %windows_amd64.zip: export runstatedir = $(prefix)
%windows_i386.zip %windows_amd64.zip: export datarootdir = $(prefix)
%windows_i386.zip %windows_amd64.zip: export docdir = $(prefix)
